...
def tournee_data(request):
    ...
    search_value = "" 
    tournees = Tournée.objects.all().order_by("id") 

    if 'search' in request.GET:
        search_value = request.GET['search']
        if search_value:  
            tournees = Tournée.objects.filter(id_chauff__nom__istartswith=search_value)
...
def delete_tournees(request):
    if request.method == "DELETE":
        data = json.loads(request.body)
        ids = data.get("ids", [])
        Tournée.objects.filter(id__in=ids).delete()
